...
class GenerateRandomNumber:
    ...
    def mixed_RNG_list(self):
        _list = []
        for i in range(0,self.count):
            if i == 0:
                #Zi-1 = Z0
                zi = self.mixed_RNG_formula(self.z0)
                ui = self.uniform_generate(zi)
                xi = self.cdf_result(ui)

                _list.append({'Zi-1': self.z0, 'Zi':zi,'Ui': ui, 'Xi': xi})
            elif i > 0:
                Zi_minus_1 = self.mixed_RNG_formula(_list[i-1]['Zi-1']) 
                zi = self.mixed_RNG_formula(Zi_minus_1)
                ui = self.uniform_generate(zi)
                xi = self.cdf_result(ui)
